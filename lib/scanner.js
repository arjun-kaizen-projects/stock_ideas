...
async function fetchRecentFilings() {
  // dateb=tomorrow so "getcurrent" returns recent filings even on weekends
  const tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate() + 1);
  const dateb = tomorrow.toISOString().slice(0, 10).replace(/-/g, '');

  // cutoff = start of 3 business days ago
  const cutoff = new Date();
  for (let bdays = 0; bdays < 3; ) {
    cutoff.setDate(cutoff.getDate() - 1);
    if (cutoff.getDay() !== 0 && cutoff.getDay() !== 6) bdays++;
  }
  console.log(`  dateb=${dateb}, cutoff=${cutoff.toISOString().slice(0,10)}`);
  ...
  for (const type of ['10-K', '10-Q']) {
    ...
    console.log(`  RSS ${type}: ${entries.length} raw entries`);
