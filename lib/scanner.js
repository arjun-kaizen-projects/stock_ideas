...
// Blocklist covers only confirmed >$100B companies where Yahoo may return null market cap
const LARGE_CAP_BLOCKLIST = new Set([
  ...
  'NFLX','BAC','ABBV','KO','PEP','MRK','CVX','CRM','ASML',
  'GS','MS','WFC','C','BLK',
]);
...
async function _doAdhocScan(targetDate) {
  ...
  for (const filing of toProcess) {
    ...
    try {
      ...
      if (quote?.marketCap && quote.marketCap > 100e9) {
        console.log(`  ✗ Skipped ${filing.companyName} — market cap ${feasibility.label} exceeds $100B`);
        continue;
      }
...
async function _doScan(targetDate) {
  ...
  // Hard valuation cap — skip companies above $100B market cap
  const MAX_MARKET_CAP = 100e9;
