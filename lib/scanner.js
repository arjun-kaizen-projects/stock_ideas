...
const LARGE_CAP_BLOCKLIST = new Set([
  'AAPL','MSFT','NVDA','AMZN','GOOGL','GOOG','META','TSLA','BRK-A','BRK-B',
  'AVGO','TSM','LLY','JPM','V','UNH','XOM','MA','ORCL','COST','HD','WMT',
  'NFLX','BAC','ABBV','KO','PEP','MRK','CVX','CRM','AMD','INTC','QCOM',
  'AMAT','LRCX','KLAC','ASML','MU','TXN','ADI','MRVL','ON',
  'GS','MS','WFC','C','BLK','SCHW',
]);

/* ─── Fetch market cap + valuation from Yahoo Finance ─────────────────────── */
async function fetchValuation(ticker) {
  ...
  try {
    ...
    const marketCap = shares && price ? shares * price : (meta.marketCap || null);
    const trailingRevenue = meta.trailingAnnualDividendRate; // not revenue — need quote endpoint
    return { price, marketCap, currency: meta.currency || 'USD' };
  } catch { return null; }
}

// Fetch market cap via Yahoo price + XBRL shares outstanding
async function fetchQuote(ticker, cik) {
  if (!ticker || ticker === 'N/A') return null;
  try {
    // Get current price from Yahoo chart API
    const chartUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(ticker)}?interval=1d&range=1d`;
    const r = await fetch(chartUrl, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 8000 });
    if (!r.ok) return null;
    const data = await r.json();
    const meta = data?.chart?.result?.[0]?.meta || {};
    ...
    // Get shares outstanding from XBRL facts
    let sharesOutstanding = null;
    let totalRevenue = null;
    if (cik) {
      try {
        const padded = String(parseInt(cik, 10)).padStart(10, '0');
        const xbrlRes = await fetch(`https://data.sec.gov/api/xbrl/companyfacts/CIK${padded}.json`, { headers: { 'User-Agent': UA }, timeout: 10000 });
        if (xbrlRes.ok) {
          const xbrl = await xbrlRes.json();
          const gaap = xbrl?.facts?.['us-gaap'] || {};
          // Shares outstanding
          const sharesData = gaap['CommonStockSharesOutstanding']?.units?.shares || [];
          const latestShares = sharesData.sort((a,b) => new Date(b.end) - new Date(a.end))[0];
          if (latestShares) sharesOutstanding = latestShares.val;
          // Revenue (trailing annual)
          const revKeys = ['Revenues', 'RevenueFromContractWithCustomerExcludingAssessedTax'];
          for (const key of revKeys) {
            const revData = (gaap[key]?.units?.USD || []).filter(e => e.form === '10-K').sort((a,b) => new Date(b.end) - new Date(a.end));
            if (revData[0]) { totalRevenue = revData[0].val; break; }
          }
        }
      } catch(_) {}
    }
    ...
    const priceToSales = (marketCap && totalRevenue) ? marketCap / totalRevenue : null;
    return { marketCap, price, sharesOutstanding, priceToSales, priceToBook: null, totalRevenue, totalCash: null, totalDebt: null, enterpriseVal: null };
  } catch { return null; }
}

function score10xFeasibility(marketCapUSD) {
  if (!marketCapUSD) return { score: 50, label: 'Unknown', flag: false };
  const b = marketCapUSD / 1e9;
  if (b < 0.5)   return { score: 100, label: `$${(marketCapUSD/1e6).toFixed(0)}M`, flag: false };
  if (b < 2)     return { score: 95,  label: `$${b.toFixed(1)}B`, flag: false };
  if (b < 5)     return { score: 88,  label: `$${b.toFixed(1)}B`, flag: false };
  if (b < 10)    return { score: 75,  label: `$${b.toFixed(1)}B`, flag: false };
  if (b < 20)    return { score: 60,  label: `$${b.toFixed(1)}B`, flag: false };
  if (b < 50)    return { score: 40,  label: `$${b.toFixed(0)}B`, flag: true  };
  if (b < 100)   return { score: 25,  label: `$${b.toFixed(0)}B`, flag: true  };
  if (b < 500)   return { score: 10,  label: `$${b.toFixed(0)}B`, flag: true  };
  return           { score: 0,   label: `$${b.toFixed(0)}B`, flag: true  };
}
...
function buildPrompt(filing, text, sector, valuation) {
...
VALUATION CONTEXT (at time of filing):
Market Cap: ${valuation ? valuation.marketCapLabel : 'Unknown'}
Price/Sales: ${valuation?.priceToSales ? valuation.priceToSales.toFixed(1)+'x' : 'N/A'}
Price/Book: ${valuation?.priceToBook ? valuation.priceToBook.toFixed(1)+'x' : 'N/A'}
Enterprise Value: ${valuation?.enterpriseVal ? '$'+(valuation.enterpriseVal/1e9).toFixed(1)+'B' : 'N/A'}
Cash: ${valuation?.totalCash ? '$'+(valuation.totalCash/1e9).toFixed(1)+'B' : 'N/A'}
10x Feasibility: ${valuation ? valuation.feasibility.label + ' market cap — ' + (valuation.feasibility.score >= 75 ? 'highly feasible' : valuation.feasibility.score >= 40 ? 'possible but harder' : 'very hard — company is already large') : 'Unknown'}

ARCHETYPE VALUATION BENCHMARKS (what the originals looked like pre-run):
SanDisk pre-run: ~$4B market cap, P/S ~1.5x, cheap vs. asset value
Micron pre-run: ~$10B market cap, P/B ~1x, trading near trough book value
Nebius pre-run: ~$2.5B market cap, EV near zero (cash-heavy), market pricing business at nothing
...
Return ONLY valid JSON (no markdown fences):
{
  ...
  "valuationVerdict": "<one sentence: is the valuation small enough to support a 10x move? Compare to archetype benchmarks.>",
  "tenxFeasibility": <0-100 score for 10x potential based purely on current market cap size>
}
...
VALUATION SCORING RULE: A company with great filing signals but a $500B+ market cap cannot realistically 10x — it would need to become a $5T company. Always factor market cap into your final assessment. tenxFeasibility: under $1B=95, $1-5B=88, $5-20B=75, $20-50B=50, $50-100B=30, $100-500B=10, over $500B=0. A high signalScore company with tenxFeasibility=0 should have shouldHighlight=false unless it is truly exceptional.
...
/* ─── Claude analysis ─────────────────────────────────────────────────────── */
async function analyzeWithClaude(filing, text, sector, valuation) {
  const prompt = buildPrompt(filing, text, sector, valuation);
...
async function _doAdhocScan(targetDate) {
  ...
  for (const filing of toProcess) {
    ...
    try {
      ...
      const [sic, quote] = await Promise.all([getSICCode(filing.cik), fetchQuote(filing.ticker, filing.cik)]);
      const sector   = classifySector(sic);
      const feasibility = score10xFeasibility(quote?.marketCap);

      if (quote?.marketCap && quote.marketCap > 50e9) {
        console.log(`  ✗ Skipped ${filing.companyName} — market cap ${feasibility.label} exceeds $50B`);
        continue;
      }

      const valuation = quote ? { ...quote, marketCapLabel: feasibility.label, feasibility } : null;
      const text     = await fetchFilingText(filing.link, filing.cik, filing.type);
      const analysis = await analyzeWithClaude(filing, text, sector, valuation);
      analysis.link     = filing.link;
      analysis.cik      = filing.cik;
      analysis.marketCap     = quote?.marketCap || null;
      analysis.marketCapLabel = feasibility.label;
      analysis.priceToSales  = quote?.priceToSales || null;
      analysis.priceToBook   = quote?.priceToBook || null;
      analysis.tenxFeasibility = analysis.tenxFeasibility ?? feasibility.score;
...
async function _doScan(targetDate) {
  ...
  // Hard valuation cap — skip companies above $50B market cap (10x would require $500B+)
  const MAX_MARKET_CAP = 50e9;
  ...
  for (const filing of toProcess) {
    ...
    try {
      ...
      // Skip if ticker is on the large-cap blocklist
      if (filing.ticker && LARGE_CAP_BLOCKLIST.has(filing.ticker.toUpperCase())) {
        console.log(`  ✗ Skipped ${filing.companyName} — on large-cap blocklist`);
        continue;
      }
      // Skip if market cap is known and too large
      if (quote?.marketCap && quote.marketCap > MAX_MARKET_CAP) {
        console.log(`  ✗ Skipped ${filing.companyName} — market cap ${feasibility.label} exceeds $50B`);
        continue;
      }
  ...
  const ARC_THRESH = 70, SCORE_THRESH = 60;
...
module.exports = { runDailyScan, getScanStatus, fetchQuote, score10xFeasibility, LARGE_CAP_BLOCKLIST };
