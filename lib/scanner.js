...
/* ─── Fetch EDGAR current filings RSS ─────────────────────────────────────── */
async function fetchTodayFilings() {
  const filings = [];
  for (const type of ['10-K', '10-Q']) {
    const url = `https://www.sec.gov/cgi-bin/browse-edgar?action=getcurrent&type=${encodeURIComponent(type)}&dateb=&owner=include&count=40&search_text=&output=atom`;
...
async function runDailyScan() {
  console.log(`[${new Date().toISOString()}] Daily SEC scan starting…`);
  ...
  let todayFilings = [];
  try {
    todayFilings = await fetchTodayFilings();
