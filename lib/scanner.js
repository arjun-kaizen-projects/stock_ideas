...
SCORING RULES: Be honest. Most companies score 20-45. Score 65+ only for genuine acceleration stories with multiple signal types firing simultaneously. Score 80+ only for SanDisk-level density. A company with >40% YoY revenue growth + expanding margins + confident management language + new market entry would score ~75-85.`;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (res.status === 429) {
      ...
      const isDaily = body.includes('exceeded your current quota') ||
                      body.includes('RESOURCE_EXHAUSTED') ||
                      body.includes('quota exceeded');
      if (isDaily) {
        const err = new Error(
          'Gemini daily quota exhausted — resets at midnight PT. ' +
          'Enable billing at console.cloud.google.com to remove the limit.'
        );
        err.quotaExhausted = true;
        throw err;
      }
      // RPM limit — retry with backoff (30s, then 60s)
      if (attempt < 2) {
        const waitSec = 30 * (attempt + 1);
        console.log(`  Gemini RPM limit hit. Retrying in ${waitSec}s (attempt ${attempt+1}/2)…`);
        await new Promise(r => setTimeout(r, waitSec * 1000));
        continue;
      }
      throw new Error(`Gemini 429 (RPM limit): ${body.substring(0,150)}`);
    }
    ...
    if (s===-1||e===-1) throw new Error('No JSON from Gemini');
...
async function _doScan() {
  ...
  const toProcess = todayFilings.filter(f => !seenLinks.has(f.link)).slice(0, 10);
  ...
  for (const filing of toProcess) {
    ...
    try {
      ...
      await new Promise(r => setTimeout(r, 5000)); // 5s gap → safely under 15 RPM free tier limit
    } catch(e) {
      console.error(`  ✗ ${filing.companyName}: ${e.message}`);
      if (e.quotaExhausted) {
        quotaExhausted = true;
        console.log('  ⚠ Daily quota exhausted — stopping early and saving results.');
      }
    }
  }
