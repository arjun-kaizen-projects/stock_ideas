...
/* ─── Fetch EDGAR filings from last 3 business days ───────────────────────── */
async function fetchRecentFilings() {
  // 5 calendar days back safely covers 3 business days across any weekend
  const cutoff = new Date();
  cutoff.setDate(cutoff.getDate() - 5);

  const filings = [];
  for (const type of ['10-K', '10-Q']) {
    const url = `https://www.sec.gov/cgi-bin/browse-edgar?action=getcurrent&type=${encodeURIComponent(type)}&dateb=&owner=include&count=100&search_text=&output=atom`;
    ...
    for (const e of entries) {
      ...
      // Skip filings older than 3 business days
