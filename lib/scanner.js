...
// Yahoo Finance v10 quote — has marketCap, trailingPE, priceToBook, revenuePerShare etc.
async function fetchQuote(ticker) {
  ...
  try {
    ...
    const priceToSales   = ks.priceToSalesTrailing12Months?.raw || null;
    ...
    const enterpriseVal  = ks.enterpriseValue?.raw || null;
    return { marketCap, priceToSales, priceToBook, totalRevenue, totalCash, totalDebt, enterpriseVal };
  } catch { return null; }
}
...
async function _doAdhocScan(targetDate) {
  ...
  for (const filing of toProcess) {
    ...
    try {
      ...
      const [sic, quote] = await Promise.all([getSICCode(filing.cik), fetchQuote(filing.ticker)]);
